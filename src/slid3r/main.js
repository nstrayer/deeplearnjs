...
function slid3r() {
  ...
  // Calculates the beta function between alpha and beta
  /**
   * @param {object} - A selection from d3
   * @return {object} - A slider
   */
  function drawSlider(sel) {
    ...
    const track = slider
      .selectAppend('line.track')
      .attr('x1', xScale.range()[0])
      .attr('x2', xScale.range()[1]);

    const trackInset = slider
      .selectAppend('line.track-inset')
      .attr('x1', xScale.range()[0])
      .attr('x2', xScale.range()[1]);

    const trackOverlay = slider
      .selectAppend('line.track-overlay')
      .attr('x1', xScale.range()[0])
      .attr('x2', xScale.range()[1]);
